...
typedef struct AnimationSprite {
  ...
  int y_difference;  // Pour ajuster le sprite et ses layers par rapport à la
                     // nouvelle position de l'animation
  ...
  bool (*linked_function)(struct Engine *e); /*fonction liée à l'animation,
       par exemple si il faut vérifier que le joueur a encore des munitions
       avant de charger le super shotgun */
...
struct Weapon {
  ...
  /*Animation*/
  animations_array
      *sprites; /**Tableau avec en i tous les sprites pour l'animation x*/
...
struct Player {
  ...
  /*Animation de l'arme*/
  int life;
  // int cooldown; //nombre d'unités de temps nécéssaires avant de tirer , 0
  // indique qu'on peut tirer double spray; //nombre d'unités de temps
  // nécéssaires avant de ne plus avoir de spray, 0 indique que le tir sera
  // parfaitement droit
  i16 subsector_id;
