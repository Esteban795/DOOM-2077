...
* It is also linked the door entity (entity_id) to the actual door struct (door_id).
*
* Door are either open or closed.  
* Door cannot be added programmatically, they are created by the map loader. The order 
* in which the doors are loaded MUST be the same on the client and server. The `door_id`
* directly corresponds to the door loading order.
*/
typedef struct {
...
/// door_state_create creates a new door state component with the given door_id and state.
