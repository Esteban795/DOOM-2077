...
door **get_doors(linedef **linedefs, int len_linedefs, int *doors_count,
  ...
  *doors_count = DOORS_COUNT;
  linedefs_tuples = malloc(
      sizeof(tuple_index) *
      len_sectors); // for each sector, have an entry (index1, index2) that are
                    // the index of collidable linedefs to open a door
  ...
  int door_index = 0;
  for (int i = 0; i < len_linedefs; i++) {
    if (is_a_door(linedefs[i])) { // a door that is not remotely controlled
  ...
  for (int i = 0; i < len_sectors; i++) {
    line1 = NULL;
    line2 = NULL;
    if (sectors[i].tag_number != 0 &&
        sectors[i].ceiling_height ==
            sectors[i].floor_height) { // sector, which is a door,is subject to
                                       // an action from another linedef, lets
                                       // find which one
      linedef *trigger_linedef = get_linedef_by_sector_tag(
          linedefs, len_linedefs,
          sectors[i].tag_number); // linedef that, when interacted with,
                                  // triggers the door
      // we then need to find the properties of the door, which is hidden in one
      // of the linedefs that are part of the sector...

      for (int j = 0; j < len_linedefs; j++) {
        if (!linedefs[j]->has_back_sidedef)
          continue; // does not have a backsidedef, useless
        if (linedefs[j]->front_sidedef->sector_id == i ||
            linedefs[j]->back_sidedef->sector_id == i) {
      ...
      // we found the two linedefs that are each side of the door, we can now
      // create the door
      set_correct_sidedefs(line1, &sector_sidedef1, &neighbor_sidedef1);
      set_correct_sidedefs(line2, &sector_sidedef2, &neighbor_sidedef2);
      door *door =
          create_door_from_linedef(sector_sidedef1, trigger_linedef->line_type);
      trigger_linedef->door =
          door; // this is linedef that will be able to trigger the door
      ...
      door_index++;
               ...
               linedefs_tuples[i].index2 != -1) {
      // 'two-sided' door
      line1 = linedefs[linedefs_tuples[i].index1];
      line2 = linedefs[linedefs_tuples[i].index2];
      set_correct_sidedefs(line1, &sector_sidedef1, &neighbor_sidedef1);
      set_correct_sidedefs(line2, &sector_sidedef2, &neighbor_sidedef2);
