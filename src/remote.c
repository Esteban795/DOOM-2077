    ...
    if (SDLNet_UDP_Recv(r->socket, r->packet) > 0) {
        ...
        while (offset + 5 < r->packet->len) {
            ...
            if (strncmp(cmd, SERVER_COMMAND_KICK, 4) == 0) {
                ...
                get_position_angle(e->p, &pos2d, &p_angle);
                ...
                if (doors_count != e->num_doors) {
                    ...
                    for (int i = 0; i < doors_count; i++) {
                        if (e->doors[i]->state != doors_states[i]) {
                            door_trigger_switch(pos2d,p_angle,e->doors[i]);
                        }
                    }
                }
                free(doors_states);
                ...
                if (lifts_count != e->len_lifts) {
                    ...
                    for (int i = 0; i < lifts_count; i++) {
                        if (e->lifts[i]->state != lifts_states[i]) {
                            lift_trigger_switch(pos2d,p_angle,e->lifts[i]);
                        }
                    }
                }
                free(lifts_states);
                ...
                if (door_id < e->num_doors && e->doors[door_id]->state == false) {
                    door_trigger_switch(pos2d,p_angle,e->doors[door_id]);
                }
                ...
                if (lift_id < e->len_lifts && e->lifts[lift_id]->state == false) {
                    lift_trigger_switch(pos2d,p_angle,e->lifts[lift_id]);
                }
...
void client_door_trigger(engine* e, uint64_t door_id) {
    ...
    if (door_trigger_switch(pos2d,pos->angle,e->doors[door_id])) {
...
void client_lift_trigger(engine* e, uint64_t lift_id) {
    ...
    if (lift_trigger_switch(pos2d,pos->angle,e->lifts[lift_id])) {
