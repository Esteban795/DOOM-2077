...
/// Respawn task
///
/// Respawn a player after some time by TPing them elsewhere and reseting their health.
///
...
void respawn_task(void* _entity_id) {
    ...
    health_ct* health = (health_ct*) world_get_component(&SERVER_STATE->world, &pid, COMPONENT_TAG_HEALTH);
    if (health == NULL) return ; // If the player does not have a position or health, cancel.
    if (health_get(health) > 0) return; // Cancel too if the player is alive.
    ...
    switch (event->tag) {
        ...
        case SERVER_PLAYER_DAMAGE_EVENT_TAG: {
            ...
            if (player_damage_event->source_entity_id != 0) {
                ...
                health_ct* src_hth = (health_ct*) world_get_component(world, &src_id, COMPONENT_TAG_HEALTH);
            ...
            if (health_get(health) <= 0) return -1; // If the player is already dead, don't trigger the death event.
