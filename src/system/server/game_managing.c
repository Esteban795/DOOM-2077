...
// Game start cooldown task, this task is responsible for the countdown before
// the game starts.
void game_start_cooldown_task(void *data) {
  ...
  game_start_event_t *game_start_event =
      ServerGameStartEvent_new(gcd->countdown);
  ...
  if (gcd->countdown == 0) {
    ...
    task_t task =
        task_new(game_start_cooldown_task, new_gcd, new_gcd->execute_at);
    task_executor_add(&SERVER_STATE->task_executor, task);
  }
}
...
void game_end_task(void *data) {
  ...
  if (gcd->countdown == 0) {
    ...
    char msg[128];
    snprintf(msg, 128, "Restarting in %d seconds...",
             SERVER_GAME_COOLDOWN_BEFORE_RESTART);
    server_chat_event_t *chat_event =
        ServerServerChatEvent_new(msg, true, true);
...
// Game restart task, this task is responsible for restarting the game after a
// reasonable cooldown.
void game_restart_task(void *_data) {
  ...
  if (SERVER_STATE->conn_count >= SERVER_MINIMUM_PLAYERS) {
    ...
    server_chat_event_t *chat_event = ServerServerChatEvent_new(
        "Not enough players to restart a game...", true, true);
    world_queue_event(&SERVER_STATE->world, (event_t *)chat_event);
  }
}

/*
 * Game managing system, this system is responsible for managing the launch and
 * end of the game.
 */
int game_managing(world_t *world, event_t *event) {
  ...
  case SERVER_PLAYER_JOIN_EVENT_TAG: {
    ...
    if (SERVER_STATE->conn_count >= SERVER_MINIMUM_PLAYERS) {
      SERVER_STATE->game_state = GAME_STATE_COOLDOWN;
      // event_t* game_start_event = event_new(SERVER_GAME_START_IN_EVENT_TAG,
      // NULL); world_push_event(world, game_start_event);
      game_cooldown_data_t *gcd = malloc(sizeof(game_cooldown_data_t));
      gcd->countdown = 5;
      INSTANT_NOW(&gcd->execute_at);
      task_t task = task_new(game_start_cooldown_task, gcd, gcd->execute_at);
  ...
  case SERVER_PLAYER_QUIT_EVENT_TAG: {
    if (SERVER_STATE->game_state == GAME_STATE_COOLDOWN &&
        SERVER_STATE->conn_count < SERVER_MINIMUM_PLAYERS) {
      SERVER_STATE->game_state = GAME_STATE_WAITING;
      server_chat_event_t *chat_event = ServerServerChatEvent_new(
          "Game aborted! Not enough players...", true, true);
      world_queue_event(world, (event_t *)chat_event);
    } else if (SERVER_STATE->game_state == GAME_STATE_RUNNING &&
               SERVER_STATE->conn_count <= 1) {
      SERVER_STATE->game_state = GAME_STATE_ENDING;
      server_chat_event_t *chat_event = ServerServerChatEvent_new(
          "Game ended! Not enough players still in game.", true, true);
      world_queue_event(world, (event_t *)chat_event);
    } else if (SERVER_STATE->game_state == GAME_STATE_ENDING &&
               SERVER_STATE->conn_count < 1) {
      SERVER_STATE->game_state = GAME_STATE_WAITING;
    }
    break;
  ...
  case SERVER_GAME_START_EVENT_TAG: {
    ...
    if (ev->countdown == 0) {
      for (int i = 0; i < SERVER_STATE->conn_count; i++) {
        ...
        weapon_ct *weapon = (weapon_ct *)world_get_component(
            world, &player, COMPONENT_TAG_WEAPON);
        if (weapon == NULL) {
          continue;
        }
        ...
        len += server_player_weapon_update(buf + len, weapon->ammunitions,
                                           weapon->mags, weapon->cooldowns);

        // Reset the health of all players.
        health_ct *health = (health_ct *)world_get_component(
            world, &player, COMPONENT_TAG_HEALTH);
        if (health == NULL) {
          continue;
        }
        health->health = 100.0;
        health->max_health = 100.0;
        len_all += server_player_health(buf_all + len_all, pid, health->health,
                                        health->max_health);

        // Reset the position of all players.
        position_ct *position = (position_ct *)world_get_component(
            world, &player, COMPONENT_TAG_POSITION);
        if (position == NULL) {
          continue;
        }
        ...
        printf("Player %lu spawned at (%f, %f, %f) with angle %f\n", pid,
               position->x, position->y, position->z, position->angle);
        len_all +=
            server_player_move(buf_all + len_all, pid, position->x, position->y,
                               position->z, position->angle);

        // Reset the statistics of all players.
        statistic_ct *statistic = (statistic_ct *)world_get_component(
            world, &player, COMPONENT_TAG_STATISTIC);
        if (statistic == NULL) {
          continue;
        }
        statistic->deaths = 0;
        ...
        int pind =
            find_conn_by_id(SERVER_STATE->conns, SERVER_STATE->conn_count, pid);
        if (pind >= 0) {
      ...
      broadcast(&SERVER_STATE->sock, SERVER_STATE->conns,
                SERVER_STATE->conn_count, buf_all, len_all);
    }
    break;
  }
