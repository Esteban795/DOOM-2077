...
int broadcast_event(world_t *world, event_t *event) {
  ...
  case SERVER_PLAYER_CHAT_EVENT_TAG: {
    ...
    display_name_ct *display_name = (display_name_ct *)world_get_component(
        world, &pid, COMPONENT_TAG_DISPLAY_NAME);
    if (display_name == NULL)
      break;
    printf("%s (%ld)> %s\n", display_name_get(display_name),
           player_chat_event->entity_id, player_chat_event->message);

    len = server_player_chat(buf, player_chat_event->entity_id,
                             player_chat_event->message);
  ...
  case SERVER_SERVER_CHAT_EVENT_TAG: {
    ...
    len = server_server_chat(buf, server_chat_event->message,
                             server_chat_event->is_broadcast,
                             server_chat_event->is_title);
    broadcast(&sock, conns, SERVER_STATE->conn_count, buf, len);
    break;
  }
  case SERVER_PLAYER_JOIN_EVENT_TAG: {
    ...
    len = server_join(buf, server_player_join_event->entity_id,
                      server_player_join_event->name);
    ...
    if (pos == NULL || health == NULL) {
      printf("SERVER_PLAYER_JOIN_EVENT_TAG > Player does not have a position "
             "or health component.\n");
    } else {
      len += server_player_move(buf + len, server_player_join_event->entity_id,
                                pos->x, pos->y, pos->z, pos->angle);
      len +=
          server_player_health(buf + len, server_player_join_event->entity_id,
                               health->health, health->max_health);
      bool *doors_states =
          (bool *)malloc(sizeof(bool) * SERVER_STATE->door_count);
      door_states_generate(world, SERVER_STATE->doors, doors_states,
                           SERVER_STATE->door_count);
      bool *lifts_states =
          (bool *)malloc(sizeof(bool) * SERVER_STATE->lift_count);
      lift_states_generate(world, SERVER_STATE->lifts, lifts_states,
                           SERVER_STATE->lift_count);
      len +=
          server_door_states(buf + len, SERVER_STATE->door_count, doors_states);
      len +=
          server_lift_states(buf + len, SERVER_STATE->lift_count, lifts_states);
      free(doors_states);
      free(lifts_states);
      char *entries[10] = {0};
      ...
      len += server_scoreboard_update(buf + len, entity_count, entries, deaths,
                                      kills);
    }
    broadcast(&sock, conns, SERVER_STATE->conn_count, buf, len);
    ...
    for (int i = 0; i < SERVER_STATE->conn_count; i++) {
      ...
      position_ct *pos = (position_ct *)world_get_component(
          world, &pid, COMPONENT_TAG_POSITION);
      health_ct *health =
          (health_ct *)world_get_component(world, &pid, COMPONENT_TAG_HEALTH);
      if (display_name == NULL)
        continue;
      outgoing->len +=
          server_join(outgoing->data + outgoing->len, conns[i].player_id,
                      display_name_get(display_name));
      if (pos == NULL)
        continue;
      outgoing->len +=
          server_player_move(outgoing->data + outgoing->len, conns[i].player_id,
                             pos->x, pos->y, pos->z, pos->angle);
      if (health == NULL)
        continue;
      outgoing->len += server_player_health(outgoing->data + outgoing->len,
                                            conns[i].player_id, health->health,
                                            health->max_health);
    }
  ...
  case SERVER_PLAYER_QUIT_EVENT_TAG: {
    server_player_quit_event_t *server_player_quit_event =
        (server_player_quit_event_t *)event;
  ...
  case SERVER_PLAYER_MOVE_EVENT_TAG: {
    ...
    // display_name_ct* display_name = (display_name_ct*)
    // world_get_component(world, &pid, COMPONENT_TAG_DISPLAY_NAME); if
    // (display_name == NULL) break; printf("%s moved to (%f, %f, %f) at angle
    // %f.\n", display_name_get(display_name), player_move_event->x,
    // player_move_event->y, player_move_event->z, player_move_event->angle);
    len = server_player_move(buf, player_move_event->entity_id,
                             player_move_event->x, player_move_event->y,
                             player_move_event->z, player_move_event->angle);
    broadcast_except(&sock, conns, SERVER_STATE->conn_count,
                     player_move_event->entity_id, buf, len);
    break;
  }
  case SERVER_PLAYER_DAMAGE_EVENT_TAG: {
    ...
    if (health == NULL)
      break;
    len = server_player_damage(buf, player_damage_event->entity_id,
                               player_damage_event->source_entity_id,
                               player_damage_event->weapon_id,
                               player_damage_event->damage);
    broadcast(&sock, conns, SERVER_STATE->conn_count, buf, len);
    break;
  }
  case SERVER_PLAYER_HEAL_EVENT_TAG: {
    ...
    if (health == NULL)
      break;
    len = server_player_heal(buf, player_heal_event->entity_id,
                             player_heal_event->gain);
    broadcast(&sock, conns, SERVER_STATE->conn_count, buf, len);
    break;
  }
  case SERVER_PLAYER_HEALTH_EVENT_TAG: {
    ...
    if (health == NULL)
      break;
    len = server_player_health(buf, player_health_event->entity_id,
                               health->health, health->max_health);
    broadcast(&sock, conns, SERVER_STATE->conn_count, buf, len);
    break;
  }
  case SERVER_PLAYER_KILL_EVENT_TAG: {
    ...
    display_name_ct *killed_name = (display_name_ct *)world_get_component(
        world, &killed, COMPONENT_TAG_DISPLAY_NAME);
    display_name_ct *killer_name = (display_name_ct *)world_get_component(
        world, &killer, COMPONENT_TAG_DISPLAY_NAME);
    if (killed_name == NULL)
      break;
    ...
    printf("%s\n", death_msg);
    len = server_player_kill(buf, player_kill_event->entity_id,
                             player_kill_event->source_entity_id,
                             player_kill_event->weapon_id);
  ...
  case SERVER_PLAYER_FIRE_EVENT_TAG: {
    ...
    broadcast_except(&sock, conns, SERVER_STATE->conn_count, ev->entity_id, buf,
                     len);
    break;
  }
  case SERVER_SCOREBOARD_UPDATE_EVENT_TAG: {
    ...
    len = server_scoreboard_update(buf, ev->entries_count, entries, ev->deaths,
                                   ev->kills);
    broadcast(&sock, conns, SERVER_STATE->conn_count, buf, len);
    break;
  }
