...
void init_animation_sprite(animation_sprite *as, patch sprite,
                           bool (*linked_function)(engine *e), weapon *w) {
  as->animation_sprite = sprite;
  as->linked_function = linked_function;
  as->y_difference = HEIGHT - sprite.header.height * Y_SCALE +
                     sprite.header.y_offset * Y_SCALE;
  set_origin(as);
  ...
  as->duration = 0;
  // printf("Default init of %s with duration
  // %d\n",sprite.patchname,as->duration);
  return;
}

animations_array *init_animations_array(engine *e, char *abbreviation,
                                        json_t *fire_frames,
                                        json_t *fire_layers, weapon *w) {
  animations_array *ar = malloc(
      3 * sizeof(animations_array)); // A priori pas besoin de plus vu qu'on a
                                     // que 2 animations
                                     // pour les armes : IDLE et animation + la
                                     // partie qui se rajoute sur l'animation

  int cooldown = 1000 / (w->fire_rate);

  // PREMIERE PARTIE : RECUPERE LES INDEX TOUS LES SPRITES RELATIF A L'ARME + LA
  // LONGUEUR DE L'ANIMATION
  int m = strlen(abbreviation);
  int indexes[10]; // J'ai supposé qu'on aurait pas plus de 10 sprites pour
                   // l'animation
  for (int i = 0; i < 10; i++) {
    indexes[i] = -1;
  }
  ...
  int i = 0;
  int special_index =
      0; // Index spécial pour rajouter des frames dans l'animation
  int special_sprite_index =
      0; // Index spécial pour rajouter des frames dans l'animation
  ...
  // Crée la partie IDLE
  animation_sprite *idle_animation_sprites_array =
      malloc(sizeof(animation_sprite));
  init_animation_sprite(idle_animation_sprites_array, sprites[indexes[0]], NULL,
                        w);
  animations_array idle_anim = {.animation_sprites_array =
                                    idle_animation_sprites_array,
                                .animation_len = 1};
  ar[IDLE] = idle_anim;

  // Crée la partie d'animation de l'animation array
  animation_sprite *fire_anim_sprites =
      malloc(sizeof(animation_sprite) * (animation_len - 1));
  ...
  animations_array fire_anim = {.animation_sprites_array = fire_anim_sprites,
                                .animation_len = animation_len - 1};
  ar[ANIMATION] = fire_anim;

  // Crée la partie de la couche qui se rajoute sur l'animation
  if (strcmp(abbreviation, "PUN") == 0) {
    animations_array fire_layer = {.animation_sprites_array = NULL,
                                   .animation_len = 0};
    ar[FIRE] = fire_layer;
  } else {
    // Crée un tableau d'animation_sprite de taille all_sprites_len -
    // animation_len pour les layers
    animation_sprite *layer_sprites =
        malloc(sizeof(animation_sprite) * (all_sprites_len - animation_len));
    ...
    animations_array fire_layer = {.animation_sprites_array = layer_sprites,
                                   .animation_len =
                                       all_sprites_len - animation_len};
    ar[FIRE] = fire_layer;

    int n = json_array_size(
        fire_frames); // Récupère le nombre de frames de l'animation qui ont un
                      // layer du fichier json
    for (int i = 0; i < n; i++) {
      // Pour chaque frame de l'animation

      // Tant que la frame d'animation suivante est la même que la frame
      // actuelle dans l'array, c'est a dire que la frame à plusieurs layers On
      // avance en indice pour récupérer tous les layers de la frame
      int j = i;
      while (i + 1 < n &&
             json_integer_value(json_array_get(fire_frames, i + 1)) ==
                 json_integer_value(json_array_get(fire_frames, i))) {
        i++;
      }
      ...
      // Alloue le tableau des index des layers de l'animation, avec pour chaque
      // case l'indice du layer a utiliser dans le tableau
      // ar[FIRE].animation_sprites_array
      int *layer_index = malloc((i - j + 1) * sizeof(int));
      int layer_index_len = i - j + 1;
      int duration = 0;
      for (int k = 0; k < i - j + 1; k++) {
        // Pour chaque layer de la frame, on récupère l'index du layer dans le
        // tableau ar[FIRE].animation_sprites_array et l'ajoute au tableau
        // layer_index
      ...
      // On récupère l'index de la frame d'animation en question depuis
      // l'origine (j)
      ...
      if (index == 0) {
        ...
        ar[ANIMATION].animation_sprites_array[index - 1].layers_index =
            layer_index;
        ar[ANIMATION].animation_sprites_array[index - 1].layer_index_len =
            layer_index_len;
        ar[ANIMATION].animation_sprites_array[index - 1].duration = duration;
      }
    }
  }
  // Initialise les positions des layers de l'animation et les décale comme il
  // faut
  animation_sprite as = ar[IDLE].animation_sprites_array[0];
  for (int k = 0; k < as.layer_index_len; k++) {
    // printf("Original pos y %f of the layer
    // %d\n",ar[FIRE].animation_sprites_array[as.layers_index[k]].wanim_origin.y,as.layers_index[k]);
    ar[FIRE].animation_sprites_array[as.layers_index[k]].wanim_origin.y =
        -ar[FIRE]
                .animation_sprites_array[as.layers_index[k]]
                .animation_sprite.header.y_offset *
            Y_SCALE +
        as.y_difference;
    // printf("Initialisation pos y %f of the layer
    // %d\n",ar[FIRE].animation_sprites_array[as.layers_index[k]].wanim_origin.y,as.layers_index[k]);
    ar[FIRE].animation_sprites_array[as.layers_index[k]].wanim_pos.y =
        ar[FIRE].animation_sprites_array[as.layers_index[k]].wanim_origin.y;
  }
  for (int i = 0; i < ar[ANIMATION].animation_len; i++) {
    ...
    for (int k = 0; k < as.layer_index_len; k++) {
      ...
      // \n",as.wanim_origin.x,as.wanim_origin.y); printf("Original pos y %d of
      // the layer
      // %d\n",ar[FIRE].animation_sprites_array[as.layers_index[k]].animation_sprite.header.y_offset
      // * Y_SCALE,as.layers_index[k]);
      ar[FIRE].animation_sprites_array[as.layers_index[k]].wanim_origin.y =
          -ar[FIRE]
                  .animation_sprites_array[as.layers_index[k]]
                  .animation_sprite.header.y_offset *
...
weapon *init_one_weapon(engine *e, int id, char *weapon_name,
                        char *abbreviation, json_t *fire_frames,
                        json_t *fire_layers, int magsize, int max_damage,
                        int min_damage, double fire_rate, double spray,
                        int ammo_bounce, int ammo_id, int type) {
  ...
  w->type = type;
  w->sprites =
      init_animations_array(e, abbreviation, fire_frames, fire_layers, w);
  return w;
}
  ...
  for (int j = 1; j < 3; j++) {
    ...
    for (int i = 0; i < aa[j].animation_len; i++) {
      ...
      printf("Animation/Layer %d de %s : %s \n", i, w->weapon_name,
             aa[j].animation_sprites_array[i].animation_sprite.patchname);
      ...
      printf("Layers : ");
      for (int k = 0; k < aa[j].animation_sprites_array[i].layer_index_len;
           k++) {
...
weapons_array *init_weapons_array(engine *en) {
  ...
  json_array_foreach(weapons, i, weapon_data) {
    ...
    const char *weapon_name =
        json_string_value(json_object_get(weapon_data, "weapon_name"));
    const char *abbreviation =
        json_string_value(json_object_get(weapon_data, "abbreviation"));
    int magsize = json_integer_value(json_object_get(weapon_data, "magsize"));
    int max_damage =
        json_integer_value(json_object_get(weapon_data, "max_damage"));
    int min_damage =
        json_integer_value(json_object_get(weapon_data, "min_damage"));
    double fire_rate =
        json_real_value(json_object_get(weapon_data, "fire_rate"));
    double spray = json_real_value(json_object_get(weapon_data, "spray"));
    int ammo_bounce =
        json_integer_value(json_object_get(weapon_data, "ammo_bounce"));
    ...
    // printf("id: %d, weapon_name: %s, abbreviation: %s, magsize: %d,
    // max_damage: %d, min_damage: %d, fire_rate: %lf, spray: %lf, ammo_bounce:
    // %d, ammo_id: %d, type:
    // %d\n",id,weapon_name,abbreviation,magsize,max_damage,min_damage,fire_rate,spray,ammo_bounce,ammo_id,type);
    weapon *new_weapon = init_one_weapon(
        en, id, (char *)weapon_name, (char *)abbreviation, fire_frames,
        fire_layers, magsize, max_damage, min_damage, fire_rate, spray,
        ammo_bounce, ammo_id, type);
    winv[i] = new_weapon;
  }
...
void idle_weapon_animation(engine *e, weapon *w, bool is_moving) {
  ...
  if (init_x + dx > origin_x + ANIMATION_WIDTH / 2 ||
      init_x + dx < origin_x - ANIMATION_WIDTH / 2) {
    dx = -dx;
    idle_anim_sprite->wanim_speed.x = dx;
  }

  if (init_y + dy > origin_y + ANIMATION_HEIGTH / 2 ||
      init_y + dy < origin_y - ANIMATION_HEIGTH / 2) {
    dy = -dy;
  ...
  draw_weapon(e, idle_anim_sprite->animation_sprite,
              idle_anim_sprite->wanim_pos.x, idle_anim_sprite->wanim_pos.y);
}

void add_fire_layer(animations_array *fire_layer, animation_sprite as,
                    int time_elapsed, engine *e) {
  if (fire_layer == NULL) {
    return;
  }
  ...
  for (int j = 0; j < layer_index_len; j++) {
    int layer_duration =
        fire_layer->animation_sprites_array[layers_index[j]].duration;
    if (time_elapsed > layer_accumulated_duration &&
        time_elapsed <= layer_accumulated_duration + layer_duration) {
      fire_layer_anim_sprite =
          &fire_layer->animation_sprites_array[layers_index[j]];
      x = fire_layer_anim_sprite->wanim_origin.x;
      y = fire_layer_anim_sprite->wanim_origin.y;
      draw_weapon(
          e,
          fire_layer->animation_sprites_array[layers_index[j]].animation_sprite,
          x, y);
    }
...
void fire_weapon_animation(engine *e, weapon *w) {
  player *p = e->p;
  animations_array *aa = w->sprites;
  animation_sprite *fire_anim =
      aa[ANIMATION].animation_sprites_array; // Tableau de toutes les animations
                                             // de l'arme
  animation_sprite *idle_anim =
      aa[IDLE].animation_sprites_array; // Idle animation
  animations_array *fire_layer = NULL;  // Layer a ajouter
  if (w->id != 0) {                     // Si c'est pas les poings
    fire_layer =
        &aa[FIRE]; // On récupère la couche qui se rajoute sur l'animation
  }
  int x; // Position x du sprite
  int y; // Position y du sprite

  int animation_len =
      aa[ANIMATION].animation_len; // Nombre d'animations de l'arme
  int time_elapsed =
      time_elapsed_in_game -
      p->t_last_shot;     // Temps écoulé depuis le début de l'animation
  ...
  if (time_elapsed < idle_anim[0].duration) {
    ...
    int fire_time_elapsed =
        time_elapsed -
        idle_anim[0].duration; // Subtract the duration of the idle animation
    if (fire_time_elapsed < fire_anim[0].duration) {
      ...
      while (fire_time_elapsed > accumulated_duration && i < animation_len &&
             fire_time_elapsed <= total_duration) {
        accumulated_duration += fire_anim[i].duration;
        i++;
      }
      // printf("Sprite affiché :
...
void update_animation(engine *e) {
  ...
  uint64_t casted_cooldown =
      (uint64_t)w->sprites->animation_duration +
      w->sprites[IDLE].animation_sprites_array[0].duration;
...
void update_weapons(engine *e) {
  ...
  player_keybind *kb = p->keybinds;
  bool fists = keys[get_key_from_action(kb, "FISTS")],
       pistol = keys[get_key_from_action(kb, "PISTOL")],
       chaingun = keys[get_key_from_action(kb, "CHAINGUN")],
       shotgun = keys[get_key_from_action(kb, "SHOTGUN")];
  if (!p->has_attacked) {
    if (fists) {
