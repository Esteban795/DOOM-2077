  ...
  if (BSP_TRAVERSE) {
    ...
    if (node_id >= SUBSECTOR_IDENTIFIER) { // leaf reached
      ...
      for (int i = 0; i < NUM_PLAYERS; i++) {
        ...
        subsector_id_ct *player_subsector_id =
            (subsector_id_ct *)world_get_component(b->engine->world, player,
                                                   COMPONENT_TAG_SUBSECTOR_ID);
        animation_ct *player_animation = (animation_ct *)world_get_component(
            b->engine->world, player, COMPONENT_TAG_ANIMATION);
        if (player_subsector_id->subsector_id == subsector_id &&
            is_point_in_FOV(player_pos->x, player_pos->y, player_pos->angle,
                            FOV, pos->x, pos->y)) {
          bool use_mirror = set_correct_animation_name(
              i, player_pos2d, player_angle, pos2d, angle,
              &player_animation->animation, position_has_moved(pos));
          patch *player_sprite = get_patch_from_name(
              b->engine->wData->sprites, b->engine->wData->len_sprites,
              ANIMATION_NAME);
          ...
          vssprite_add(
              player_pos2d, player_pos->angle, pos2d, player_sprite, use_mirror,
              i, animation_get_shot_filter_duration_left(player_animation) > 0);
        }
      }
...
void update_players_subsectors(bsp *b) {
  for (int i = 0; i < NUM_PLAYERS; i++) {
    ...
    i16 subsector_id =
        get_subsector_id_from_pos(b->root_node_id, b->nodes, pos_v);
