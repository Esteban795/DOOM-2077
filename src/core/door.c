...
// trigger switch for all doors in the list, but they need to not be switching
// and be in their initial state (closed or open)
bool door_trigger_switch(door *d) {
  if (d != NULL) {
    ...
    if (!d->is_switching && d->state == d->init_state) {
      d->is_switching = true;
      switched = true;
      add_sound_to_play(DOOR_OPEN_SOUND, d->sector->center_pos.x,
                        d->sector->center_pos.y);
    }
    if (d->next_door != NULL) {
...
void door_update(door *d, int DT) {
  ...
  if (d->state != d->init_state) { // update door's internal timer to know if it
                                   // needs to switch
    d->time_elapsed += DT;
  ...
  if (d->is_switching) {
    double multiplier;
    // if the door is going up, we need to add to the height, if it's going
    // down, we need to subtract
