...
void door_update(door *d, int DT) {
  ...
  if (d->is_switching) { // time elapsed, should we switch ?
    ...
    int multiplier = d->is_open ? -1 : 1; // determines which side the wall is going
    int new_height = d->sector->ceiling_height + (d->speed / 100 * d->delta_height * DT * multiplier);
    d->sector->ceiling_height = max(min_height, min(max_height, new_height)); // make sure we don't get under or over the target heights
    if (new_height == min_height || new_height == max_height) { // wall is at the top or bottom
      d->is_switching = false;
      d->is_open = !d->is_open; // transition is done , switch state
      if (d->function == OPEN_STAY_OPEN || d->function == CLOSE_STAY_CLOSED) { // one time transition
        d->speed = NO_SPEED;
      }
    }
  }
